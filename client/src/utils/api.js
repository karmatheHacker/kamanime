...
export function SEARCH_TORRENT(query) {
  return `${BASE_URL_NYAA}/?q=${encodeURIComponent(query)}&sort=seeders&order=desc&page=1&category=anime`
}

export function SEARCH_TORRENT_TOSHO(query, page = 1) {
  return `${TOSHO}/json?qx=1&q=${encodeURIComponent(query)}&page=${page}`
}
