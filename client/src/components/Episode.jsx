...
export default function Episode({
  ...
  const hasAnidbIds = !!(data?.aids)

  // Primary: Tosho by AniDB IDs (precise, requires AniZip mappings)
  const {
    isLoading: isLoadingToshoId,
    data: toshoEpsById,
    error: errorToshoId
  } = useGetoToshoEpisodes(
    active && hasAnidbIds ? data?.quality : null,
    data?.aids,
    data?.eids ? data.eids : null
  )

  // Fallback: Tosho text search (when no AniDB IDs available)
  // Prefer English title; strip subtitle after ':' since Nyaa releases use short names
  const buildFallbackQuery = () => {
    if (!active || hasAnidbIds) return null
    const raw = anime?.english || anime?.romaji || ''
    // "Jujutsu Kaisen: Shimetsu Kaiyuu" → "Jujutsu Kaisen"
    return raw.includes(':') ? raw.split(':')[0].trim() : raw
  }
  const fallbackQuery = buildFallbackQuery()
  const {
    isLoading: isLoadingToshoText,
    torrents: toshoEpsByText,
    error: errorToshoText
  } = useToshoTracker(fallbackQuery)

  const isLoading = hasAnidbIds ? isLoadingToshoId : isLoadingToshoText
  const toshoEps = hasAnidbIds ? toshoEpsById : toshoEpsByText
  const error = hasAnidbIds ? errorToshoId : errorToshoText
          ...
          <div className="mt-3 flex flex-col gap-y-2">
            ...
            {!isLoading && torrentData?.length === 0 && (
              <p className="font-space-mono text-red-500">
                No torrents found{!hasAnidbIds ? ' — try the manual search box above with a shorter title' : ''}
              </p>
            )}
