...
export default function Player(query) {
  ...
  const { vlcPath, backendPort, serverUrl } = useZenshinContext()
  ...
  useEffect(() => {
    setDiscordRPC()
    return () => {
      api.setDiscordRpc({ details: 'Browsing Anime' })
    }
  }, [discordRpcActivity])
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(
        `${serverUrl}/add/${encodeURIComponent(magnetURI)}`
      )
      console.log(response)
      // Step 2: Set the video source for streaming
      setVideoSrc(`${serverUrl}/stream/${encodeURIComponent(magnetURI)}`)
      // Step 3: Set the subtitle source
      setSubtitleSrc(`${serverUrl}/subtitles/${encodeURIComponent(magnetURI)}`)
    } catch (error) {
  ...
  const getFiles = async () => {
    ...
    api.saveToSettings('currentAnime', temp_obj)

    try {
      console.log('Inside getFiles')

      const response = await axios.get(
        `${serverUrl}/metadata/${encodeURIComponent(magnetURI)}`
      )
  ...
  const handleVlcStream = async () => {
    toast.info('VLC is not available in the web version.')
  }
  ...
  const handleStreamBrowser = (episode) => {
    ...
    let temp_obj = {
      episodeName: episode,
      streamUrl: `${serverUrl}/streamfile/${encodeURIComponent(magnetURI)}/${encodeURIComponent(episode)}`,
      ...loc.state
    }
    ...
    setVideoSrc(
      `${serverUrl}/streamfile/${encodeURIComponent(magnetURI)}/${encodeURIComponent(episode)}`
    )
  }
  ...
  const handleStreamVlc = async (episode) => {
    toast.info('VLC is not available in the web version. Use the browser player instead.')
  }

  const stopEpisodeDownload2 = async (episode) => {
    try {
      console.log(
        `${serverUrl}/deselect/${encodeURIComponent(magnetURI)}/${encodeURIComponent(episode)}`
      )
  ...
  const handleRemoveTorrent = async () => {
    try {
      ...
      await axios.delete(`${serverUrl}/remove/${encodeURIComponent(magnetURI)}`)
  ...
  return (
    <div className="mb-32 flex items-center justify-center px-2 font-space-mono sm:px-8">
      <div className="w-full">
        ...
        {/* We basically do this to prevent video player re-render */}
        {currentEpisode && (
          <StreamStatsEpisode
            magnetURI={magnetURI}
