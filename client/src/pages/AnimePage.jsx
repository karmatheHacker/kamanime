...
export default function AnimePage() {
  ...
  function setDiscordRPC() {
    ...
    api.setDiscordRpc && api.setDiscordRpc(activityDetails)
  }

  useEffect(() => {
    setDiscordRPC()
    return () => {
      api.setDiscordRpc && api.setDiscordRpc({ details: 'Browsing Anime' })
    }
  }, [animeData, episodesWatched, malIdData])

  if (isLoading) return <CenteredLoader />

  // errorMappings is non-critical (zenshinSupabase/AniZip supplementary data) — fail silently
  if (errorMappings) console.warn('AniZip mappings unavailable:', errorMappings?.message)

  if (errorMalId) {
    toast.error('Error fetching Anime', {
      icon: <ExclamationTriangleIcon height="16" width="16" color="#ffffff" />,
      description: `Couldn't fetch anime: ${errorMalId?.message}`,
      classNames: {
        title: 'text-rose-500'
      }
    })
  ...
  // Use AniZip episode list if available, otherwise generate a numbered list from AniList episode count
  if (episodesAnizip) {
    animeEpisodes = episodesAnizip
  } else if (data?.episodes) {
    animeEpisodes = Array.from({ length: data.episodes }, (_, i) => ({
      epNum: i + 1,
      title: `Episode ${i + 1}`,
      thumbnail: null,
      airdate: null,
